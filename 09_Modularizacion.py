...
#Vamos  modularizar el conversor de monedas
...
def conversor(tipo_pesos, valor_dolar):
    pesos = input("Ingresa la cantidad de pesos: " + tipo_pesos + ": " )  # Le pedimos al usuario que ingrese la cantidad de pesos
    pesos = float(pesos)    #Ese  numero lo convertimos en un numero flotante, recordar que todo lo que ingresa por input ers un String
    USD = valor_dolar  #Asignaos el valor de la tasa de cambio en una constante
    cambio = pesos / USD     #Realizamso la conversion diviendo la cantidad de pesos entre la tasa de cambio USD
    cambio = round(cambio, 2) #Vamso a usar la funcion round() para asignar la cantida de deciames que queremos ver,de lo contrario mostrara muchos
    cambio = str(cambio) #Convertimos el valor de cambio a String para poder imprimirlo
    print("El cambio fue realizado con éxito :" + "$" + cambio + " Dólares")    #Imprimimos


if opcion == 1:
    conversor("Argentinos", 208)
elif opcion == 2:
    conversor("Uruguayos", 45.50)
elif opcion == 3:
    conversor("Chilenos", 3800)
else:
    print("No ingresaste un numero valido")